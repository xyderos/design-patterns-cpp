...
test_flyweight::test_flyweight_should_keep_consistent_shared_state(void)
{
    ...
    auto first_candidate = fw.business_logic(unique_state("either", "other"));
    auto second_candidate = fw.business_logic(unique_state("other", "either"));

    CPPUNIT_ASSERT_EQUAL(first_candidate.substr(0, first_candidate.find(":")), second_candidate.substr(0, second_candidate.find(":")));
